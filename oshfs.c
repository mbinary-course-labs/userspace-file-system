 ...
 * value  : block_no, st,name ,  content is the value of the first block_no of data
 */
struct filenode {
	block_no_t block_no ;
    int  content;
...
static int zfs_write(const char *path, const char *buffer, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	int num = (offset+size)/CONTENT_SIZE;  //think twice
